...
while 1 == 1:
    ...
    if response == IphoneMac:
        ...
        if ((16 <= datetime.now().hour <= 21) and (datetime.today().weekday() < 5) and (CurrentTime - LastSMSSentAt) > (
                60 * 60 * 12)):
            ...
            if LastSMSSentAt != 0:
                print "--Last alert sent:", time.strftime("%H Hours, %M Minutes, %S Seconds ago",
                                                          time.gmtime(CurrentTime - LastSMSSentAt))
